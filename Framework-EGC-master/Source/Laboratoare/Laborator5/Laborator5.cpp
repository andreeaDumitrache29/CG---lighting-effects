...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_K)) {
		// TODO : translate the camera up
		neg += deltaTime * 10;
		projectionMatrix = glm::perspective(RADIANS(neg), 5.0f, 0.01f, 200.0f);
	}
	...
	if (window->KeyHold(GLFW_KEY_N)) {
		// TODO : translate the camera up
		latime += deltaTime * 10;
		lungime += deltaTime * 10;
		projectionMatrix = glm::ortho(-latime, latime, -lungime, lungime, 0.01f, 200.0f);
	}
