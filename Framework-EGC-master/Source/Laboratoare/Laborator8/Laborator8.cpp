...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0, 0.5457, 0.874));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.745, 0.21, 1));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1, 0.5, 0));
	}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int cutoff_loc = glGetUniformLocation(shader->program, "cut_off");
	glUniform1f(cutoff_loc, cut_off);
	...
	int loc_spot = glGetUniformLocation(shader->program, "spot");
	glUniform1f(loc_spot, spot);
